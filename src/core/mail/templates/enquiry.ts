...
import { company, support, physical_address } from "../../data/system-info";
                                                            ...
                                                            <tr>
                                                                <td style="font-size:0px;padding:10px 10px;word-break:break-word;">
                                                                    <div style="font-family:'Helvetica Neue',Arial,sans-serif;font-size:15px;line-height:22px;color:#555;">
                                                                        Hi, <br/><br/>
                                                                        ${payload.message} <br/><br/>
                                                                        ${ payload.type == 'foreign_uni' ?
                                                                            `
                                                                            Min Budget: ${payload.budget.min}<br/>
                                                                            Max Budget: ${payload.budget.max}<br/><br/>

                                                                            Preffered Countries:
                                                                            <ul>
                                                                                <li>${payload.country_1}</li>
                                                                                ${!IfEmpty(payload.country_2) ? `<li>`+payload.country_2+`</li>` : "" }
                                                                                ${!IfEmpty(payload.country_3) ? `<li>`+payload.country_3+`</li>` : "" }
                                                                            </ul>
                                                                            
                                                                            Preffered courses:
                                                                            <ul>
                                                                                <li>${payload.courses[0]?.name}</li>
                                                                                ${!IfEmpty(payload.courses[1]) ? `<li>`+payload.courses[1]?.name+`</li>` : "" }
                                                                                ${!IfEmpty(payload.courses[2]) ? `<li>`+payload.courses[2]?.name+`</li>` : "" }
                                                                            </ul>` : ""
                                                                        }

                                                                        ${ !IfEmpty(payload.other_info) ? ` Other Info: <br/> ${ payload.other_info }` : "" }

                                                                        <br/><br/>
                                                                        
                                                                        Reach ${CapString(payload.fullname)} via email: ${payload.email} or phone: ${payload.phone}.
